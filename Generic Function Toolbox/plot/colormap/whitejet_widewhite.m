...
% ----------------------------------------------------------------------- %
function J = whitejet_widewhite(m, white_ratio)
...
m = m * (white_ratio - 1) / white_ratio;

% Colors
color_palette = [1 0 0;     % Red
                 1 1 0;     % Yellow
                 1 1 1;     % White
                 0 1 1;     % Cyan
                 0 0 1];    % Blue
...
% Make the gradients
J = zeros(m,3);
J(color_samples,:) = color_palette(1:end,:);
...
J = flipud(J);
J = [J(1 : end/2, :); ones(m/white_ratio, 3); J(end/2+1 : end, :)];
