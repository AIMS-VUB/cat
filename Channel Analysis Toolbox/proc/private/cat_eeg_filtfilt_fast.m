function X = cat_eeg_filtfilt_fast(varargin)
...
elseif nargin == 4
    ...
    B = cat_eeg_design_fir(N,F,sqrt(M)); A = 1; % note: we use the sqrt() because we run forward and backward
else
    help cat_eeg_filtfilt_fast;
    return;
end
...
if A == 1
    ...
    % filter, reverse
    X = cat_eeg_filter_fast(B,A,X); X = X(length(X):-1:1,:);
    % filter, reverse
    X = cat_eeg_filter_fast(B,A,X); X = X(length(X):-1:1,:);
