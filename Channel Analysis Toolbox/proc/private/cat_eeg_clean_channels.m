function signal = cat_eeg_clean_channels(signal,corr_threshold,noise_threshold,window_len,max_broken_time,num_samples,subset_size, ignore_channels)
...
if signal.srate > 100
    ...
    B = cat_eeg_design_fir(100,[2*[0 45 50]/signal.srate 1],[1 1 0 0]);
    for c=signal.nbchan:-1:1
        X(:,c) = cat_eeg_filtfilt_fast(B,1,signal.data(c,:)'); end
...
if ~(isfield(signal.chanlocs,'X') && isfield(signal.chanlocs,'Y') && isfield(signal.chanlocs,'Z') &&...
    all([length([signal.chanlocs.X]),length([signal.chanlocs.Y]),length([signal.chanlocs.Z])] > length(signal.chanlocs)*0.5))
  error('clean_channels:bad_chanlocs','To use this function most of your channels should have X,Y,Z location measurements.');
end
...
if exist('OCTAVE_VERSION', 'builtin') == 0
    P = cat_eeg_hlp_microcache('cleanchans',@calc_projector,locs,num_samples,subset_size);
else
...
removed_channels = removed_channels & ~ismember({signal.chanlocs.labels}, ignore_channels)';
...
%    subset = randsample(1:size(locs,2),subset_size,stream);
    tmp(subset,:) = real(cat_eeg_sphericalSplineInterpolate(locs(:,subset),locs))';
    rand_samples{k} = tmp;
end
