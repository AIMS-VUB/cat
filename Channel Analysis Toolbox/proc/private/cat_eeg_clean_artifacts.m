function [EEG,HP,BUR] = cat_eeg_clean_artifacts(EEG,varargin)
...
cat_eeg_hlp_varargin2struct(varargin,...
    ...
    {'ignore_channels', 'IgnoredChannels'}, {});
...
if ~strcmp(chancorr_crit,'off') || ~strcmp(line_crit,'off') %#ok<NODEF>
    ...
    if strcmp(line_crit,'off')
        line_crit = 100; end    
    try 
        EEG = cat_eeg_clean_channels(EEG,chancorr_crit,line_crit,[],channel_crit_maxbad_time, ...
          [], [], ignore_channels); 
    catch e
