...
%      pars.colormap    string to select one of the predefined matlab colormaps,
...
if nargin>2
  if isfield(pars,'scale') && ~isempty(pars.scale)
    scal=pars.scale;
    if length(scal)==1
      scal=[-scal scal];
    end   
...
[minmin,~,meanmin]=mindis(coor(:,1),coor(:,2));
...
for chan=1:n
  ...
  plot_elec_empty_lowres(zp,locs(:,2:3),scal,indd(chan),resolution);
...
axes('Position',[0.12 0.01 .7/.88 .99]);
...
axis([ 0 1 0 1]);
if ~isempty(mytext)
    text(.0, .9,mytext,'fontsize',fontsize,'fontweight','bold');
end
...
function plot_elec_empty_lowres(z,loc,~,chan,resolution)
